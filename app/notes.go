...
func Notes(ctx context.Context, auth *AuthHandler) http.Handler {
	db := must(sql.Open(legacyconversions.RiskilyAssumeTrustedSQL("sqlite"), legacyconversions.RiskilyAssumeTrustedSQL("./notes.db")))
	nh := &notesHandler{safesql.NewDB(db), auth}
